...
public class Compete {
    ...
    protected final Set<SchedulerTask> scheduledTasks = ConcurrentHashMap.newKeySet();
        ...
        if (config.contains("competitions") && config.getConfigurationSection("competitions") != null) {
            for (String id : config.getConfigurationSection("competitions").getKeys(false)) {
                ...
                if (cfg.isEnabled()) {
                    ...
                    kkfish.log(plugin.getMessageManager().getMessageWithoutPrefix("log.competition_load", "Loading competition configuration: %s", id));
                }
            }
        }
    }
    ...
    public void createScoreboard(String competitionId) {
        ...
        try {
            ...
            kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("competition_scoreboard_create_error", "Failed to create scoreboard: %s", e.getMessage()));
        }
    }
    ...
    public void updateScoreboard(String competitionId) {
        ...
        try {
            ...
            kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("competition_scoreboard_update_error", "Failed to update scoreboard: %s", e.getMessage()));
        }
    }
    ...
    public class ScheduledCompetition {
        ...
        private ZoneId getTimeZone() {
            ...
            try {
                ...
                kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("timezone_invalid", "Invalid timezone '%s', using system default", timezone));
                return ZoneId.systemDefault();
            }
        }
        ...
        private void scheduleRecurring() {
            try {
                ...
                if (parts.length < 3) {
                    kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("competition_schedule_parse_error", "Failed to parse recurring competition time: %s - insufficient parts", config.getSchedule()));
                    return;
                }
                ...
                kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("competition_schedule_parse_error", "Failed to parse recurring competition time: %s - %s", config.getSchedule(), e.getMessage()));
            }
        }
        ...
        private void scheduleSpecificDate() {
            try {
                ...
                if (delay <= 0) {
                    kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("log.competition_date_passed", "Specified competition date has passed: %s", config.getSchedule()));
                    return;
                }
                ...
                final BukkitRunnable runnable = new BukkitRunnable() {
                    ...
                    public void run() {
                        if (config.getMinPlayers() > 0 && Bukkit.getOnlinePlayers().size() < config.getMinPlayers()) {
                            kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("competition_cancelled_insufficient_players", "Competition %s cancelled due to insufficient players", config.getName()));
                            return;
                        }
                ...
                kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("competition_date_parse_error", "Failed to parse specific date competition time: %s - %s", config.getSchedule(), e.getMessage()));
            }
        }
        ...
        private void scheduleCompetitionTask(BukkitRunnable runnable, long delay, boolean isRecurring) {
            ...
            task = SchedulerUtil.runSyncDelayed(plugin, wrapper, delay / 50);
            taskHolder[0] = task;
        ...
        private void scheduleCompetitionNotification(long delay) {
            ...
            try {
                if (notificationDelay > 0) {
                    SchedulerTask notificationTask = SchedulerUtil.scheduleTask(plugin, new BukkitRunnable() {
                        @Override
                        public void run() {
                            try {
                ...
                kkfish.log("§e" + plugin.getMessageManager().getMessageWithoutPrefix("competition_notification_schedule_error", "Failed to schedule competition start notification task: %s", e.getMessage()));
            }
        }
    }
    ...
    public Set<SchedulerTask> getScheduledTasks() {
