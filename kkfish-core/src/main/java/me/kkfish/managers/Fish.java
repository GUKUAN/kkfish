...
public class Fish {
    ...
    private final Map<UUID, SchedulerTask> activeProgressSchedulers = new ConcurrentHashMap<>();
    private final Map<UUID, SchedulerTask> biteCheckSchedulers = new ConcurrentHashMap<>();
    ...
    private final Map<UUID, SchedulerTask> floatingEffectTasks = new ConcurrentHashMap<>();
    private final Map<UUID, SchedulerTask> trajectoryTasks = new ConcurrentHashMap<>();
    ...
    public Fish(kkfish plugin) {
        this.plugin = plugin;
        // logger removed via getConsoleSender migration
    ...
    private void startChargeProgressTask(Player player, String rodName) {
        ...
        try {
            activeProgressSchedulers.put(playerId, SchedulerUtil.runEntityTaskTimer(plugin, player, progressTask, 0, 1));
    ...
    public void stopCharging(Player player, boolean isOver100Percent) {
        ...
        activeChargeTasks.remove(playerId);
        SchedulerTask progressScheduler = activeProgressSchedulers.remove(playerId);
        if (progressScheduler != null) {
            progressScheduler.cancel();
            config.debugLog("成功取消进度更新任务 for player: " + player.getName());
        }
        activeProgressTasks.remove(playerId);
    ...
    public void cleanup() {
        ...
        for (SchedulerTask task : activeProgressSchedulers.values()) {
            task.cancel();
        }
        for (SchedulerTask task : biteCheckSchedulers.values()) {
            task.cancel();
        }
        for (SchedulerTask task : floatingEffectTasks.values()) {
            task.cancel();
        }
        for (SchedulerTask task : trajectoryTasks.values()) {
            task.cancel();
        }
        ...
        activeProgressSchedulers.clear();
        biteCheckTasks.clear();
        biteCheckSchedulers.clear();
    ...
    private void spawnParticleEffects(Location location) {
        ...
        try {
            ...
            kkfish.log("§e" + "Failed to spawn particle effects: " + e.getMessage());
            try {
                double offsetX = config.getFishEmergeParticleOffsetX();
                double offsetY = config.getFishEmergeParticleOffsetY();
                double offsetZ = config.getFishEmergeParticleOffsetZ();
    ...
    public void endSession(Player player) {
        ...
        SchedulerTask floatTask = floatingEffectTasks.remove(playerId);
        if (floatTask != null) {
            floatTask.cancel();
        }
        
        SchedulerTask trajTask = trajectoryTasks.remove(playerId);
        if (trajTask != null) {
            trajTask.cancel();
        }
    ...
    static void safeTeleport(org.bukkit.entity.Entity entity, org.bukkit.Location loc) {
        try {
            java.lang.reflect.Method m = entity.getClass().getMethod("teleportAsync", org.bukkit.Location.class);
            m.invoke(entity, loc);
        } catch (Exception e) {
            entity.teleport(loc);
        }
